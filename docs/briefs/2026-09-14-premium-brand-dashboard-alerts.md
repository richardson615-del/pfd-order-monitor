...
2. **Shell updates without a website** — pick one distribution path (Q6 in §4). Facts on each:
   ...
   **2a. Decision (Nick, 2026-09-15): MDM.** Build for it:
   - Produce a **signed release APK and AAB** on every shell change via `android/build-apk.sh`, named `premium-orders-<appVersionCode>.apk`, attached to the GitHub release for that PR — the MDM console uploads from there. Keep the existing signing key (`pfd-orders` alias); a new key would make every tablet see a different app.
   - The shell must behave under MDM kiosk (single-app lock task mode): `startUrl /dashboard`, portrait, no dependence on the Android back button or system bars, survives reboot with auto-launch (MDM handles launch; the app handles wake lock and re-auth). Remove any in-app "install" or "download" affordance.
   - Sessions must outlive an MDM-pushed update: an APK update restarts the app; Supabase cookies persist in Chrome's profile, so re-login is not required — verify on a real tablet and record in the PR.
   - `install.html` becomes staff-only enrolment notes (how to enrol a tablet into the MDM), no APK link for restaurants; keep `/app/pfd-orders.apk` only until the MDM hosts the build, then remove it.
   - MDM vendor choice, enrolment method (zero-touch/QR/afw#), and policy set (kiosk, keep-awake, Wi-Fi profile, Chrome managed policy pre-granting notifications for `pfd-order-monitor.vercel.app` via `NotificationsAllowedForUrls`, which removes even the one tap in C3) are configured in the MDM console, outside this repo — list them in the PR body as follow-ups for Nick.
   Independently of the MDM, the app itself adds one safety net: `GET /api/version` also returns `min_shell_version` (from an env var, default `0`); the TWA passes its `appVersionCode` via a `?shell=` param on `startUrl`; if the shell is below the minimum, the dashboard shows a non-blocking amber line "This tablet needs an update from Premium — we'll handle it" and the heartbeat records `shell_version` so the CRM can list tablets that need the shell update. No restaurant action is ever requested.
...
6. Shell distribution after C1 — **DECIDED 2026-09-15: MDM-managed tablets.** Build C4 §2 for the MDM path only (see C4 §2a).
